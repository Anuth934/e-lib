...
public class UserListBooks extends HttpServlet {
	...
	private void doMethod(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if(searchUsingId) {
			...
			res.sendRedirect(req.getContextPath() + "/user/book_details.jsp");
			//redirect
			...
			res.sendRedirect(req.getContextPath() + "/user/books.jsp");
